...
//		System.out.println(provider.queryTrain("曲阜东", "南京南", Calendar.getInstance()));
		System.out.println(provider.orderTicket("南京南", "嘉兴南", "G41", Calendar.getInstance(), "二等座", 1, 1,2,3));
//		System.out.println(Integer.toBinaryString(2147483647));
	...
	public AutoResult orderTicket(String src, String target, String tid, Calendar date, String seats_type, int account,
		...
		synchronized (pool.intern(src)) {
			...
			FindIterable<Document> findIterable=collection.find(Filters.and(Filters.gte("date", start),Filters.lt("date", end)));
			MongoCursor<Document> cursor2=findIterable.iterator();
			Document train=cursor2.next();
			if (train==null) {
				System.out.println(src+" "+target+" "+seats_type);
			}
			...
			AggregateIterable<Document> aggregateIterable=collection.aggregate(Arrays.asList(
					...
					Aggregates.match(Filters.and(Filters.eq("ticket.stype", Integer.toString(seats2stype(seats_type))),Filters.bitsAllSet("ticket.ticket", mask))),
			...
			temp.stream().forEach(d->{
				Document document=(Document) d.get("ticket");
				Object ttemp=document.get("ticket");
				if (ttemp instanceof Long) {
					long init=(long) ttemp;
					init=init^new Long(mask);
					document.replace("ticket", init);
				}else {
					int init=(int) ttemp;
					init=init^new Long(mask).intValue();
					document.replace("ticket", init);
				}
